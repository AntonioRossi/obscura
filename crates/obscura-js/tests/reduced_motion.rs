...
#[cfg(feature = "render")]
#[test]
fn reduced_motion_invalidates_host_and_iframe_css_geometry() {
    let mut rt = ObscuraJsRuntime::new();
    rt.set_dom(obscura_dom::parse_html("<body></body>"));
    rt.run_page_init();
    rt.evaluate(r#"(() => {
        ...
        globalThis.widths = () => [document.querySelector('div').getBoundingClientRect().width,
            frame.contentDocument.querySelector('div').getBoundingClientRect().width];
    })()"#).unwrap();
    assert_eq!(rt.evaluate("widths()").unwrap(), json!([100,100]));
    rt.set_reduced_motion(true);
    assert_eq!(rt.evaluate("widths()").unwrap(), json!([40,40]));
    rt.set_reduced_motion(false);
    assert_eq!(rt.evaluate("widths()").unwrap(), json!([100,100]));
}
